...
// MARK: - HDR Heatmap (float-component, extended-range)

public func drawHeatmapHDR(points: [LocationPoint],
                           zoom: Int, ox: Double, oy: Double, w: Int, h: Int,
                           palette: HeatmapPalette, intensity: Float = 1.0,
                           rotation: Double = 0,
                           centerPx: Double = 0, centerPy: Double = 0) -> CGImage? {
    var pixelPoints: [(x: Double, y: Double)] = []
    if rotation != 0 {
        let cosR = cos(rotation)
        let sinR = sin(rotation)
    ...
    if pixelPoints.isEmpty { return nil }
    ...
    var heatBuf = [Float](repeating: 0, count: w * h * 4)
    ...
    for gy in 0..<gh {
        for gx in 0..<gw {
            ...
            for py in y0..<y1 {
                for px in x0..<x1 {
                    let pidx = (py * w + px) * 4
                    heatBuf[pidx + 0] = r * ai  // premultiplied, may exceed 1.0
                    heatBuf[pidx + 1] = g * ai
                    heatBuf[pidx + 2] = b * ai
                    heatBuf[pidx + 3] = ai
                }
    ...
    return heatBuf.withUnsafeMutableBytes { ptr -> CGImage? in
...
// MARK: - HDR Wallpaper Generation

/// Generates an HDR wallpaper as a CIImage with extended-range heatmap.
/// The base map tiles are SDR; the heatmap layer uses float components with
/// values > 1.0 for EDR glow on XDR/OLED displays. Returns nil on failure.
public func generateMapImageHDR(lat: Double, lon: Double, zoom: Int,
                                width w: Int, height h: Int,
                                heatmapPoints: [LocationPoint]?,
                                theme: MapTheme = Themes.cyberpunk,
                                intensity: Float = 1.0,
                                rotation: Double = 0) -> CIImage? {
    let ts = Config.tileSize
    ...
    // Render tiles into 8-bit sRGB context (SDR)
    guard let tileCtx = CGContext(
        data: nil, width: renderW, height: renderH,
        bitsPerComponent: 8, bytesPerRow: 0,
        space: CGColorSpaceCreateDeviceRGB(),
    ...
    // Build the SDR base image (handle rotation)
    let baseCGImage: CGImage
    if rotation == 0 {
        guard let img = tileCtx.makeImage() else { return nil }
        baseCGImage = img
    } else {
        guard let tileImage = tileCtx.makeImage(),
              let ctx = CGContext(
                  data: nil, width: w, height: h,
                  bitsPerComponent: 8, bytesPerRow: 0,
                  space: CGColorSpaceCreateDeviceRGB(),
    ...
    // CIImage from the SDR base — CIFilter will auto-convert color spaces during compositing
    let baseCI = CIImage(cgImage: baseCGImage)

    // No heatmap? Return the SDR base as-is.
    guard let points = heatmapPoints, !points.isEmpty else {
        return baseCI
    }

    // Render HDR heatmap (float buffer in extendedLinearSRGB, values > 1.0 for EDR glow)
    guard let hdrHeatmap = drawHeatmapHDR(
        points: points, zoom: zoom, ox: ox, oy: oy, w: w, h: h,
        palette: theme.heatmap, intensity: intensity,
        rotation: rotation, centerPx: cx, centerPy: cy
    ) else {
        return baseCI
    }

    let heatCI = CIImage(cgImage: hdrHeatmap)

    // Additive compositing: heatmap light ADDS to the base, producing values > 1.0
    // This is what creates the actual EDR glow on HDR displays
    let blended: CIImage?
    switch theme.blend {
    case .screen, .normal:
        // Additive blend — heatmap pixels with values > 1.0 push total above SDR,
        // creating genuine EDR headroom that OLED displays render as brighter-than-white
        let addFilter = CIFilter(name: "CIAdditionCompositing")!
        addFilter.setValue(heatCI, forKey: kCIInputImageKey)
        addFilter.setValue(baseCI, forKey: kCIInputBackgroundImageKey)
        blended = addFilter.outputImage
    case .multiply:
        // For light themes: multiply darkens, no HDR glow needed
        let mulFilter = CIFilter(name: "CIMultiplyBlendMode")!
        mulFilter.setValue(heatCI, forKey: kCIInputImageKey)
        mulFilter.setValue(baseCI, forKey: kCIInputBackgroundImageKey)
        blended = mulFilter.outputImage
    }

    // Crop to base image extent (blend filters can produce infinite extent)
    return blended?.cropped(to: baseCI.extent)
}
